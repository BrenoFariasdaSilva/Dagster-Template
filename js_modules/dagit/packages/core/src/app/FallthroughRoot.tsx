...
const FinalRedirectOrLoadingRoot = () => {
    ...
    return (
      <Box flex={{direction: 'row', justifyContent: 'center'}} style={{paddingTop: '100px'}}>
        <Box flex={{direction: 'row', alignItems: 'center', gap: 16}}>
          ...
          <div style={{color: Colors.Gray600}}>Carregando definições...</div>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <Box padding={{vertical: 64}}>
      <NonIdealState
        icon="no-results"
        title={repoWithNoJob ? 'Sem empregos' : 'Sem definições'}
        description={
          repoWithNoJob
            ? 'Suas definições foram carregadas, mas nenhum trabalho foi encontrado.'
            : 'Adicione um trabalho para começar..'
        }
        action={
