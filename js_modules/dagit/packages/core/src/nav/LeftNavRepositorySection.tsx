  ...
  const listContent = () => {
      ...
      return (
        <EmptyState>
          <Box flex={{direction: 'column', gap: 8}} padding={{top: 12}}>
            <span style={{fontSize: '16px', fontWeight: 500}}>Sem definições</span>
            <Body>Selecione um local de código para ver uma lista de trabalhos</Body>
          </Box>
        </EmptyState>
      );
    }
    ...
    return (
      <EmptyState>
        <Box flex={{direction: 'column', gap: 8}} padding={{top: 12}}>
          <span style={{fontSize: '16px', fontWeight: 500}}>Sem definições</span>
          <Body>Ao adicionar um local de código, suas definições aparecerão aqui</Body>
        </Box>
      </EmptyState>
    );
  };
