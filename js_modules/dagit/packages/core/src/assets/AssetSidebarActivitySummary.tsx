  ...
  return (
    <>
      ...
      {asset.autoMaterializePolicy && (
        <SidebarSection title="Política de auto materialização">
          <Box margin={{horizontal: 24, vertical: 12}} flex={{gap: 12, alignItems: 'flex-start'}}>
            <Body style={{flex: 1}}>
      ...
      {loadedPartitionKeys.length > 1 ? null : (
        <>
         <SidebarSection
            title={!isSourceAsset ? 'Materialização da última execução' : 'Observação da última execução'}
          >
            {displayedEvent ? (
              ...
              >
                {!isSourceAsset ? `Não foram encontradas materializações` : `Não foram encontradas observações`}
              </Box>
            )}
          </SidebarSection>
          <SidebarSection
            title={!isSourceAsset ? 'Etiquetas do sistema de materialização' : 'Etiquetas do sistema de observação'}
            collapsedByDefault
          >
            {displayedEvent ? (
              <div style={{margin: -1, maxWidth: '100%', overflowX: 'auto'}}>
      ...
      <SidebarSection title="Parcelas de metadados">
