...
export const ERROR_INVALID_ASSET_SELECTION =
  `Os ativos só podem ser materializados em conjunto se forem definidos em` +
  ` o mesmo local de código e compartilham um espaço de partição ou formam um` +
  ` gráfico no qual os ativos raiz compartilham o mesmo particionamento.`;
...
function optionsForButton(scope: AssetsInScope, liveDataForStale?: LiveData): LaunchOption[] {
    ...
    return [
      {
        ...
        label: `Materializar selecionado${countOrBlank(assets)}${isAnyPartitioned(assets) ? '…' : ''}`,
  ...
  options.push({
    ...
    label:
      assets.length > 1 && !scope.skipAllTerm
        ? `Materializar tudo${isAnyPartitioned(assets) ? '…' : ''}`
        : `Materializar${isAnyPartitioned(assets) ? '…' : ''}`,
    hasMaterializePermission,
  });
  ...
  if (liveDataForStale) {
    ...
    options.push({
      ...
      label: `Propagar alterações${countOrBlank(missingOrStale)}`,
      hasMaterializePermission,
      ...
      <Box flex={{alignItems: 'center'}}>
        ...
        <Popover
          ...
          content={
            <Menu>
              <MenuItem
                text="Abrir na barra de lançamento"
      ...
      try {
        await confirm({
          ...
          description: (
            <>
              <div>
                A materialização desses ativos pode falhar porque os ativos upstream listados abaixo têm
