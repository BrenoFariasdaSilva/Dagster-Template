...
export const CodeLocationMenu: React.FC<{locationNode: WorkspaceRepositoryLocationNode}> = ({
  ...
  if (
    ...
    libsMenuItem = (
      <MenuItem icon="info" text="Visualizar biblioteca do dagster" onClick={() => setLibsIsOpen(true)} />
    );
    libsDialog = (
  ...
  return (
    <>
      <Popover
        ...
        content={
          <Menu>
            <MenuItem icon="info" text="Visualizar Configuração" onClick={() => setConfigIsOpen(true)} />
            {libsMenuItem}
          </Menu>
        }
      >
...
export const CodeLocationConfigDialog: React.FC<{
  ...
  return (
    <Dialog
      title="Configuração da localização do código"
      icon="info"
      isOpen={isOpen}
...
export const DagsterLibrariesDialog: React.FC<{
  ...
  return (
    <Dialog
      title="Biblioteca de versões do dagster"
      icon="info"
      isOpen={isOpen}
      onClose={() => setIsOpen(false)}
      style={{width: '600px'}}
