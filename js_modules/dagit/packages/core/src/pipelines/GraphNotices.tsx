...
export const EmptyDAGNotice: React.FC<{isGraph: boolean; nodeType: 'asset' | 'op'}> = ({
  ...
  return (
    <CenteredContainer>
      <NonIdealState
        icon="no-results"
        title={isGraph ? 'Gráfico vazio' : 'Trabalho vazio'}
        description={
          <div>
            Este {isGraph ? 'gráfico' : 'Trabalho'} está vazio. {capitalize(nodeType)}s aparecerão aqui quando forem adicionados às suas definições.
          </div>
        }
      />
    </CenteredContainer>
