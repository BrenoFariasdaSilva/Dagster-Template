  ...
  return (
    <Box flex={{direction: 'row', gap: 8, alignItems: 'center'}}>
      <Button disabled={disabled} onClick={onClick}>
        Remover configuração extra
      </Button>
      {disabled ? (
        <Box flex={{direction: 'row', gap: 4, alignItems: 'center'}}>
          <Icon name="check_circle" color={Colors.Green500} />
          Nenhuma configuração extra removida
        </Box>
      ) : null}
    </Box>
  );
...
const ScaffoldConfigButton = ({
  ...
  const onClick = async () => {
    await confirm({
      title: 'Armação precisa de configuração',
  ...
  return (
    <Box flex={{direction: 'row', gap: 8, alignItems: 'center'}}>
      <Button disabled={disabled} onClick={onClick}>
         Armação precisa de configuração
      </Button>
      {disabled ? (
        <Box flex={{direction: 'row', gap: 4, alignItems: 'center'}}>
          <Icon name="check_circle" color={Colors.Green500} />
...
const ExpandDefaultButton = ({
  ...
  const onClick = async () => {
    await confirm({
      title: 'Armação com todas as configurações padrao',
      description: (
        <div>
          Ao clicar em confirmar, qualquer campo de configuração não especificado será automaticamente
          executado com os valores padrão. Terá de alterar os valores de forma adequada..
        </div>
      ),
    });
    onExpandDefaults();
  ...
  return (
    <Box flex={{direction: 'row', gap: 8, alignItems: 'center'}}>
      <Button disabled={disabled} onClick={onClick}>
      Armação com todas as configurações padrao
      </Button>
      {disabled ? (
        <Box flex={{direction: 'row', gap: 4, alignItems: 'center'}}>
          <Icon name="check_circle" color={Colors.Green500} />
...
export const RunPreview: React.FC<RunPreviewProps> = (props) => {
  ...
  const itemsIn = (parents: string[], items: {name: string; isRequired: boolean}[]) => {
    ...
    if (!boxes.length) {
      return <ItemsEmptyNotice>Nada para mostrar.</ItemsEmptyNotice>;
    }
    return boxes;
  };
    ...
    <SplitPanelContainer
      ...
      first={
        <ErrorListContainer>
          ...
          <Section>
            <SectionTitle>Ações de configuração</SectionTitle>
        ...
        <>
          <div style={{overflowY: 'scroll', width: '100%', height: '100%'}}>
            <RuntimeAndResourcesSection>
              <Section>
                <SectionTitle>Tempo de execução</SectionTitle>
                <ItemSet>
                  {itemsIn(
                    [],
            ...
            <Section>
              <SectionTitle>{launchpadType === 'asset' ? 'Ativos (Ops)' : 'Ops'}</SectionTitle>
              <ItemSet>
                {itemsIn(
                  [hasOps ? 'ops' : 'solids'],
