...
export const OverviewJobsRoot = () => {
  useTrackPageView();
  useDocumentTitle('Visão Geral | Empregos');
    ...
    if (!filteredBySearch.length) {
      ...
      return (
        <Box padding={{top: 20}}>
          <NonIdealState
            icon="search"
            title="Nenhum emprego"
            description={
              anyReposHidden
                ? 'Não foram encontrados horários nos locais de código selecionados'
                : 'Não foram encontrados empregos nas suas definições'
            }
          />
        </Box>
      );
  ...
  return (
    <Box flex={{direction: 'column'}} style={{height: '100%', overflow: 'hidden'}}>
      <PageHeader
        title={<Heading>Empregos</Heading>}
