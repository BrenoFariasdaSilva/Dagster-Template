  ...
  return (
    <DividerContainer style={{height: `${height}px`, top: `-${DATE_TIME_HEIGHT}px`}}>
      ...
      <DividerLines>
        ...
        {now >= start && now <= end ? (
          <>
            <NowMarker style={{left: nowLeft}}>Agora</NowMarker>
  ...
  const content = () => {
    if (loading) {
      return (
        <Box flex={{direction: 'row', gap: 8, alignItems: 'center'}}>
          ...
          {includesTicks ? 'Carregando execuções e ticks agendados' : 'Carregando execuções'}
        </Box>
      );
    }
    ...
    return (
      <Box flex={{direction: 'column', gap: 12, alignItems: 'center'}}>
        <div>
          {includesTicks
            ? 'Não há execuções ou marcações agendadas neste período de tempo.'
            : 'Nenhuma corrida neste período de tempo.'}
        </div>
