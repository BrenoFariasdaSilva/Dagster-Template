...
export const RunActionsMenu: React.FC<{
  ...
  return (
    <>
      <JoinedButtons>
        <AnchorButton to={`/runs/${run.id}`}>Visualizar execução</AnchorButton>
        <Popover
          content={
            <Menu>
              <MenuItem
                tagName="button"
                text={loading ? 'Carregando configuração...' : 'Visualizar configuração...'}
              ...
              <>
                ...
                >
                  <MenuLink
                    text="Abrir plataforma de lançamento..."
                    disabled={!infoReady || !run.hasReExecutePermission}
                    icon="edit"
              ...
              <MenuExternalLink
                text="Download depuração de arquivo"
        ...
        content={
          <Menu>
            {canTerminateAny ? (
              <MenuItem
                icon="cancel"
                text={`Terminar ${terminateableIDs.length} ${
                  terminateableIDs.length === 1 ? 'executar' : 'execuções'
                }`}
            ...
            {canDeleteAny ? (
              <MenuItem
                icon="delete"
                intent="danger"
                text={`Excluir ${deleteableIDs.length} ${
                  deleteableIDs.length === 1 ? 'executar' : 'execuções'
                }`}
            ...
            {canReexecuteAny ? (
              <>
                <MenuItem
                  icon="refresh"
                  text={`Re-executar ${reexecutableRuns.length} ${
                    reexecutableRuns.length === 1 ? 'execução' : 'execuções'
                  }`}
                ...
                <MenuItem
                  icon="refresh"
                  text={`Re-executar ${reexecuteFromFailureRuns.length} ${
                    reexecuteFromFailureRuns.length === 1 ? 'execução' : 'execuções'
                  } com falhas`}
