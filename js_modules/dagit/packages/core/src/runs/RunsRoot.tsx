    ...
    <Page>
      ...
      {currentTab === 'queued' && canSeeConfig ? (
        ...
        >
          <Alert
            intent="info"
            title={<Link to="/config#run_coordinator">Ver a configuração da fila</Link>}
          />
          <QueueDaemonAlert />
        </Box>
      ) : null}
          ...
          renderError={(error: ApolloError) => {
            ...
            return (
              ...
              >
                ...
                <NonIdealState
                  icon="warning"
                  title={badRequest ? 'Filtros de execução inválidos' : 'Erro inesperado'}
                  description={
                    badRequest
                      ? 'Os filtros de execução especificados não são válidos. Verifique os filtros e tente novamente.'
                      : 'Um erro inesperado ocorreu. Verifique o console para obter detalhes.'
                  }
                />
              </Box>
            );
