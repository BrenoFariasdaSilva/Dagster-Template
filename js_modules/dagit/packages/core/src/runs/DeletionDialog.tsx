...
export const DeletionDialog = (props: Props) => {
      ...
      case 'initial':
        return (
          <Group direction="column" spacing={8}>
            <div>{`${count} ${count === 1 ? 'execução' : 'execuções'} será deletado.`}</div>
            <div>
              A exclusão de execuções não impedirá que continuem a ser executadas e pode resultar em
            ...
            <div>Deseja continuar com a exclusão?</div>
          </Group>
        );
      case 'deleting':
      case 'completed':
        ...
        return (
          <>
            ...
            <Button intent="danger" onClick={mutate}>
              {`Sim, excluir ${`${count} ${count === 1 ? 'execução' : 'execuções'}`}`}
            </Button>
            {terminatableCount ? (
              <Button intent="primary" onClick={onTerminateInstead}>
                {`Terminate ${`${terminatableCount} ${
                  terminatableCount === 1 ? 'execução' : 'execuções'
                }`} instead`}
              </Button>
            ) : null}
          </>
  ...
  const completionContent = () => {
    ...
    if (state.step === 'deleting') {
      return <div>Por favor, não feche a janela ou navegue durante a exclusão.</div>;
    }
        ...
        {successCount ? (
          <Group direction="row" spacing={8} alignItems="center">
            ...
            <div>{`Successfully deleted ${successCount} ${
              successCount === 1 ? 'execução' : 'execuções'
            }.`}</div>
          </Group>
        ) : null}
        {errorCount ? (
          <Group direction="column" spacing={8}>
            <Group direction="row" spacing={8} alignItems="center">
              ...
              <div>{`Could not delete ${errorCount} ${errorCount === 1 ? 'execução' : 'execuções'}.`}</div>
            </Group>
            <ul>
