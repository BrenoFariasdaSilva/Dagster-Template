...
export const ReexecutionDialog = (props: Props) => {
      ...
      case 'initial':
        ...
        const message = () => {
          if (reexecutionStrategy === ReexecutionStrategy.ALL_STEPS) {
            return (
              <span>
                {`${count} ${count === 1 ? 'execução' : 'execuções'} será re-executado `}
                <strong>com todas as etapas</strong>. Deseja continuar?
              </span>
            );
          }
          return (
            <span>
              {`${count} ${count === 1 ? 'execução' : 'execuções'} será re-executado `}
              <strong>com falha</strong>. Deseja continuar?
            </span>
          );
        };
        ...
        return (
          <Group direction="column" spacing={8}>
            ...
            {state.step === 'reexecuting' ? (
              <NavigationBlock message="Reexecução em curso, por favor não navegue ainda." />
            ) : null}
          </Group>
        );
      default:
        ...
        return (
          <>
            ...
            <Button intent="primary" onClick={mutate}>
              {`Re-executar ${`${count} ${count === 1 ? 'executar' : 'execuções'}`}`}
            </Button>
          </>
        );
      case 'reexecuting':
  ...
  const completionContent = () => {
    ...
    if (state.step === 'reexecuting') {
      return <div>Por favor, não feche a janela ou navegue para fora durante a re-execução.</div>;
    }
    ...
    <Dialog
      ...
      title={
        reexecutionStrategy === ReexecutionStrategy.ALL_STEPS
          ? 'Re-executar execuções'
          : 'Re-executar execuções após falha'
      }
