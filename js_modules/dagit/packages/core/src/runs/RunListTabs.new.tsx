...
const getDocumentTitle = (selected: ReturnType<typeof useSelectedRunsTab>) => {
  switch (selected) {
    case 'all':
      return 'Executar | Todas as execuções';
    case 'done':
      return 'Executar | Feito ';
    case 'in-progress':
      return 'Executar | Em progresso ';
    case 'queued':
      return 'Executar | Fila de espera';
    case 'scheduled':
      return 'Executar | Programado';
    default:
      return 'Executar';
  }
};
...
export const useRunListTabs = (filter: RunsFilter = {}) => {
  ...
  const tabs = (
    <JoinedButtons>
      ...
      >
        Em file de espera ({queuedCount ?? 'indeterminate'})
      </Button>
      <Button
      ...
      >
        Em progresso ({inProgressCount ?? 'indeterminate'})
      </Button>
      <Button
