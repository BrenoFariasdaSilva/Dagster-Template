      ...
      case 'initial':
        if (!count) {
          return (
            <Group direction="column" spacing={16}>
              <div>Nenhuma execução selecionada para rescisão.</div>
              <div>As execuções que você selecionou podem já ter terminado a execução.</div>
            </Group>
          );
        }
        ...
        return (
          <Group direction="column" spacing={16}>
            <div>
              {`${count} ${
                count === 1 ? 'execução' : 'execuções'
            } será encerrada. Você deseja continuar?`}
            </div>
            <div>
              {showCheckbox ? (
                <>
                  <Checkbox
                    ...
                    size="small"
                    label="Forçar rescisão imediatamente"
                  ...
                  {state.mustForce ? (
                    <Box flex={{display: 'flex', direction: 'row', gap: 8}} margin={{top: 8}}>
                      ...
                      <div>
                        <strong>Atenção:</strong> recursos computacionais criados por execuções não podem ser
                        limpo.
                      </div>
                    </Box>
                  ) : null}
      ...
      case 'completed':
        ...
        return (
          <Group direction="column" spacing={8}>
            <div>{state.mustForce ? 'Forçando rescisão…' : 'rescisão…'}</div>
            <ProgressBar intent="primary" value={Math.max(0.1, value)} animate={value < 1} />
            {state.step === 'terminating' ? (
              <NavigationBlock message="Encerramento em andamento, por favor, não saia ainda." />
            ) : null}
          </Group>
        );
      default:
        ...
        return (
          <>
            ...
            <Button intent="danger" onClick={mutate}>
              {`${state.mustForce ? 'Rescisão forçada para' : 'Encerrar'} ${`${count} ${
                count === 1 ? 'exeçucão' : 'execuções'
              }`}`}
            </Button>
          </>
        );
        ...
        {errorCount ? (
          <Group direction="column" spacing={8}>
            <Group direction="row" spacing={8} alignItems="flex-start">
              ...
              <div>
                {state.mustForce
                  ? `Não foi possível forçar a rescisão ${errorCount} ${
                      errorCount === 1 ? 'execução' : 'execuções'
                    }:`
                  : `Não foi possível solicitar a rescisão para ${errorCount} ${
                      errorCount === 1 ? 'execução' : 'execuções'
                    }:`}
              </div>
            </Group>
