      ...
      <div>
        ...
        <Box margin={{vertical: 32}}>
          {anyFilter ? (
            <NonIdealState
              icon="run"
              title="Não há execuções correspondentes"
              description="Não foram encontradas execuções para este filtro."
            />
          ) : (
            <NonIdealState
              icon="run"
              title="Não foram encontradas execuções"
              description={
                <Box flex={{direction: 'column', gap: 12}}>
                  <div>Ainda não lançou nenhuma execução.</div>
      ...
      <Table>
        <thead>
          <tr>
            ...
            <th style={{width: 90}}>Código da execução</th>
            <th>{anyPipelines ? 'Emprego / Condutas' : 'Emprego'}</th>
            <th style={{width: 90}}>Código Instantâneo</th>
            <th style={{width: 190}}>Calendário</th>
